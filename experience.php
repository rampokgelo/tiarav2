<?php
$title ="Experiences";
include 'templates/header.php';
 ?>
 <div class="banner-hero">
   <div class="banner-hero-item" style="background-image:url('images/bg-exp.jpg')">
     <div class="overlay-base"></div>
      <div class="caption-hero">
        <div class="container">
          <h1>Experiences</h1>
        </div>
      </div>
   </div>
 </div>
 <div class="section">
   <div class="container">
     <ul class="tabs">
       <li class="tab"><a class="active" href="#serv">Reliability Services</a></li>
       <li class="tab"><a href="#tech">Reliability Technology</a></li>
       <li class="tab"><a href="#comp">Reliability Competency</a></li>
     </ul>
  </div>
  <div id="serv">
    <div class="hide-xs">
      <script src='https://api.mapbox.com/mapbox-gl-js/v0.38.0/mapbox-gl.js'></script>
        <link href='https://api.mapbox.com/mapbox-gl-js/v0.38.0/mapbox-gl.css' rel='stylesheet' />
        <div id='map' style='width: 100%; height: 500px;'></div>
       <script>
         mapboxgl.accessToken = 'pk.eyJ1IjoicmFtcG9rZ2VsbyIsImEiOiJjajVkbnlvcG0wbzFqMzNvNjgybWk2a3hwIn0.nBiU6cWxmYz3PjjIvrn75A';
         var map = new mapboxgl.Map({
         container: 'map',
         style: 'mapbox://styles/rampokgelo/cj5do371n0mgr2robfwt4x9xo'
         });
         map.scrollZoom.disable();
       </script>
    </div>
     <section class="section grey lighten-4">
       <div class="container">
         <div class="row">
           <div class="col s12 l4">
             <div class="card small">
               <div class="card-image card-image waves-effect waves-block waves-light">
                 <img class="activator responsive-img" src="images/sample-card-logo.jpg">
               </div>
               <div class="card-content">
                 <h3 class="card-title blue-text activator activator">Operating Deflection Shape (ODS) for Machinery Troubleshooting</h3>
               </div>
               <div class="card-reveal">
                  <span class="card-title"><i class="icon-close right"></i></span>
                  <p class="blue-text">
                    Vibration Condition Monitoring Surveillance at PT. Conocophillips Indonesia
                  </p>
                  <p>
                    Routine Condition Monitoring of Rotating and Electrical Equipment including Vibration, Partial Discharge and TAN Delta Analysis
                    <br />
                    <small class="text-muted">2009-2020</small>
                  </p>
                </div>
             </div>
           </div>
           <div class="col s12 l4">
             <div class="card small">
               <div class="card-image card-image waves-effect waves-block waves-light">
                 <img class="activator responsive-img" src="images/sample-card-logo.jpg">
               </div>
               <div class="card-content">
                 <h3 class="card-title blue-text activator">Vibration Condition Monitoring Surveillance at PT. Conocophillips Indonesia</h3>
               </div>
               <div class="card-reveal">
                  <span class="card-title"><i class="icon-close right"></i></span>
                  <p class="blue-text">
                    Vibration Condition Monitoring Surveillance at PT. Conocophillips Indonesia
                  </p>
                  <p>
                    Routine Condition Monitoring of Rotating and Electrical Equipment including Vibration, Partial Discharge and TAN Delta Analysis
                    <br />
                    <small class="text-muted">2009-2020</small>
                  </p>
                </div>
             </div>
           </div>
           <div class="col s12 l4">
             <div class="card small">
               <div class="card-image card-image waves-effect waves-block waves-light">
                 <img class="activator responsive-img" src="images/sample-card-logo.jpg">
               </div>
               <div class="card-content">
                 <h3 class="card-title blue-text activator">Operating Deflection Shape (ODS) for Machinery Troubleshooting</h3>
               </div>
               <div class="card-reveal">
                  <span class="card-title"><i class="icon-close right"></i></span>
                  <p class="blue-text">
                    Vibration Condition Monitoring Surveillance at PT. Conocophillips Indonesia
                  </p>
                  <p>
                    Routine Condition Monitoring of Rotating and Electrical Equipment including Vibration, Partial Discharge and TAN Delta Analysis
                    <br />
                    <small class="text-muted">2009-2020</small>
                  </p>
                </div>
             </div>
           </div>
           <div class="col s12 l4">
             <div class="card small">
               <div class="card-image card-image waves-effect waves-block waves-light">
                 <img class="activator responsive-img" src="images/sample-card-logo.jpg">
               </div>
               <div class="card-content">
                 <h3 class="card-title blue-text activator">Operating Deflection Shape (ODS) for Machinery Troubleshooting</h3>
               </div>
               <div class="card-reveal">
                  <span class="card-title"><i class="icon-close right"></i></span>
                  <p class="blue-text">
                    Vibration Condition Monitoring Surveillance at PT. Conocophillips Indonesia
                  </p>
                  <p>
                    Routine Condition Monitoring of Rotating and Electrical Equipment including Vibration, Partial Discharge and TAN Delta Analysis
                    <br />
                    <small class="text-muted">2009-2020</small>
                  </p>
                </div>
             </div>
           </div>
           <div class="col s12 l4">
             <div class="card small">
               <div class="card-image card-image waves-effect waves-block waves-light">
                 <img class="activator responsive-img" src="images/sample-card-logo.jpg">
               </div>
               <div class="card-content">
                 <h3 class="card-title blue-text activator">Operating Deflection Shape (ODS) for Machinery Troubleshooting</h3>
               </div>
               <div class="card-reveal">
                  <span class="card-title"><i class="icon-close right"></i></span>
                  <p class="blue-text">
                    Vibration Condition Monitoring Surveillance at PT. Conocophillips Indonesia
                  </p>
                  <p>
                    Routine Condition Monitoring of Rotating and Electrical Equipment including Vibration, Partial Discharge and TAN Delta Analysis
                    <br />
                    <small class="text-muted">2009-2020</small>
                  </p>
                </div>
             </div>
           </div>
           <div class="col s12 l4">
             <div class="card small">
               <div class="card-image card-image waves-effect waves-block waves-light">
                 <img class="activator responsive-img" src="images/sample-card-logo.jpg">
               </div>
               <div class="card-content">
                 <h3 class="card-title blue-text activator">Operating Deflection Shape (ODS) for Machinery Troubleshooting</h3>
               </div>
               <div class="card-reveal">
                  <span class="card-title"><i class="icon-close right"></i></span>
                  <p class="blue-text">
                    Vibration Condition Monitoring Surveillance at PT. Conocophillips Indonesia
                  </p>
                  <p>
                    Routine Condition Monitoring of Rotating and Electrical Equipment including Vibration, Partial Discharge and TAN Delta Analysis
                    <br />
                    <small class="text-muted">2009-2020</small>
                  </p>
                </div>
             </div>
           </div>
           <div class="col s12 l4">
             <div class="card small">
               <div class="card-image card-image waves-effect waves-block waves-light">
                 <img class="activator responsive-img" src="images/sample-card-logo.jpg">
               </div>
               <div class="card-content">
                 <h3 class="card-title blue-text activator">Operating Deflection Shape (ODS) for Machinery Troubleshooting</h3>
               </div>
               <div class="card-reveal">
                  <span class="card-title"><i class="icon-close right"></i></span>
                  <p class="blue-text">
                    Vibration Condition Monitoring Surveillance at PT. Conocophillips Indonesia
                  </p>
                  <p>
                    Routine Condition Monitoring of Rotating and Electrical Equipment including Vibration, Partial Discharge and TAN Delta Analysis
                    <br />
                    <small class="text-muted">2009-2020</small>
                  </p>
                </div>
             </div>
           </div>
         </div>
         <div class="text-center">
           <a href="#" class="waves-effect waves-light btn btn-large btn-flat btn-outline-primary">Load More</a>
         </div>
         <br />
       </div>
     </section>
     <section>
         <div class="container">
           <h2 class="title"><span></span>See Our Client Testimonials</h2>
           <div class="slick-wrapper">
             <div class="ben-slick padd-slick">
               <div class="text-center">
                 <p>
                   Quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariaturLorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation
                 </p>
                 <i class="icon-quote amber-text"></i>
                 <p>
                   <strong>Pertamina</strong>
                 </p>
               </div>
               <div class="text-center">
                 <p>
                   Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariaturLorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation
                 </p>
                 <i class="icon-quote amber-text"></i>
                 <p>
                   <strong>Pertamina</strong>
                 </p>
               </div>
               <div class="text-center">
                 <p>
                   Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariaturLorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation
                 </p>
                 <i class="icon-quote amber-text"></i>
                 <p>
                   <strong>Pertamina</strong>
                 </p>
               </div>
               <div class="text-center">
                 <p>
                   Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariaturLorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation
                 </p>
                 <i class="icon-quote amber-text"></i>
                 <p>
                   <strong>Pertamina</strong>
                 </p>
               </div>
             </div>
           </div>
         </div>

     </section>
  </div>

  <div id="tech">
    <div class="hide-xs">
      <script src='https://api.mapbox.com/mapbox-gl-js/v0.38.0/mapbox-gl.js'></script>
        <link href='https://api.mapbox.com/mapbox-gl-js/v0.38.0/mapbox-gl.css' rel='stylesheet' />
        <div id='map' style='width: 100%; height: 500px;'></div>
       <script>
         mapboxgl.accessToken = 'pk.eyJ1IjoicmFtcG9rZ2VsbyIsImEiOiJjajVkbnlvcG0wbzFqMzNvNjgybWk2a3hwIn0.nBiU6cWxmYz3PjjIvrn75A';
         var map = new mapboxgl.Map({
         container: 'map',
         style: 'mapbox://styles/rampokgelo/cj5do371n0mgr2robfwt4x9xo'
         });
         map.scrollZoom.disable();
       </script>
    </div>
     <section class="section grey lighten-4">
       <div class="container">
         <div class="row">
           <div class="col s12 l4">
             <div class="card small">
               <div class="card-image card-image waves-effect waves-block waves-light">
                 <img class="activator responsive-img" src="images/sample-card-logo.jpg">
               </div>
               <div class="card-content">
                 <h3 class="card-title blue-text activator">Operating Deflection Shape (ODS) for Machinery Troubleshooting</h3>
               </div>
               <div class="card-reveal">
                  <span class="card-title"><i class="icon-close right"></i></span>
                  <p class="blue-text">
                    Vibration Condition Monitoring Surveillance at PT. Conocophillips Indonesia
                  </p>
                  <p>
                    Routine Condition Monitoring of Rotating and Electrical Equipment including Vibration, Partial Discharge and TAN Delta Analysis
                    <br />
                    <small class="text-muted">2009-2020</small>
                  </p>
                </div>
             </div>
           </div>
           <div class="col s12 l4">
             <div class="card small">
               <div class="card-image card-image waves-effect waves-block waves-light">
                 <img class="activator responsive-img" src="images/sample-card-logo.jpg">
               </div>
               <div class="card-content">
                 <h3 class="card-title blue-text activator">Vibration Condition Monitoring Surveillance at PT. Conocophillips Indonesia</h3>
               </div>
               <div class="card-reveal">
                  <span class="card-title"><i class="icon-close right"></i></span>
                  <p class="blue-text">
                    Vibration Condition Monitoring Surveillance at PT. Conocophillips Indonesia
                  </p>
                  <p>
                    Routine Condition Monitoring of Rotating and Electrical Equipment including Vibration, Partial Discharge and TAN Delta Analysis
                    <br />
                    <small class="text-muted">2009-2020</small>
                  </p>
                </div>
             </div>
           </div>
           <div class="col s12 l4">
             <div class="card small">
               <div class="card-image card-image waves-effect waves-block waves-light">
                 <img class="activator responsive-img" src="images/sample-card-logo.jpg">
               </div>
               <div class="card-content">
                 <h3 class="card-title blue-text activator">Operating Deflection Shape (ODS) for Machinery Troubleshooting</h3>
               </div>
               <div class="card-reveal">
                  <span class="card-title"><i class="icon-close right"></i></span>
                  <p class="blue-text">
                    Vibration Condition Monitoring Surveillance at PT. Conocophillips Indonesia
                  </p>
                  <p>
                    Routine Condition Monitoring of Rotating and Electrical Equipment including Vibration, Partial Discharge and TAN Delta Analysis
                    <br />
                    <small class="text-muted">2009-2020</small>
                  </p>
                </div>
             </div>
           </div>
           <div class="col s12 l4">
             <div class="card small">
               <div class="card-image card-image waves-effect waves-block waves-light">
                 <img class="activator responsive-img" src="images/sample-card-logo.jpg">
               </div>
               <div class="card-content">
                 <h3 class="card-title blue-text activator">Operating Deflection Shape (ODS) for Machinery Troubleshooting</h3>
               </div>
               <div class="card-reveal">
                  <span class="card-title"><i class="icon-close right"></i></span>
                  <p class="blue-text">
                    Vibration Condition Monitoring Surveillance at PT. Conocophillips Indonesia
                  </p>
                  <p>
                    Routine Condition Monitoring of Rotating and Electrical Equipment including Vibration, Partial Discharge and TAN Delta Analysis
                    <br />
                    <small class="text-muted">2009-2020</small>
                  </p>
                </div>
             </div>
           </div>
           <div class="col s12 l4">
             <div class="card small">
               <div class="card-image card-image waves-effect waves-block waves-light">
                 <img class="activator responsive-img" src="images/sample-card-logo.jpg">
               </div>
               <div class="card-content">
                 <h3 class="card-title blue-text activator">Operating Deflection Shape (ODS) for Machinery Troubleshooting</h3>
               </div>
               <div class="card-reveal">
                  <span class="card-title"><i class="icon-close right"></i></span>
                  <p class="blue-text">
                    Vibration Condition Monitoring Surveillance at PT. Conocophillips Indonesia
                  </p>
                  <p>
                    Routine Condition Monitoring of Rotating and Electrical Equipment including Vibration, Partial Discharge and TAN Delta Analysis
                    <br />
                    <small class="text-muted">2009-2020</small>
                  </p>
                </div>
             </div>
           </div>
           <div class="col s12 l4">
             <div class="card small">
               <div class="card-image card-image waves-effect waves-block waves-light">
                 <img class="activator responsive-img" src="images/sample-card-logo.jpg">
               </div>
               <div class="card-content">
                 <h3 class="card-title blue-text activator">Operating Deflection Shape (ODS) for Machinery Troubleshooting</h3>
               </div>
               <div class="card-reveal">
                  <span class="card-title"><i class="icon-close right"></i></span>
                  <p class="blue-text">
                    Vibration Condition Monitoring Surveillance at PT. Conocophillips Indonesia
                  </p>
                  <p>
                    Routine Condition Monitoring of Rotating and Electrical Equipment including Vibration, Partial Discharge and TAN Delta Analysis
                    <br />
                    <small class="text-muted">2009-2020</small>
                  </p>
                </div>
             </div>
           </div>
           <div class="col s12 l4">
             <div class="card small">
               <div class="card-image card-image waves-effect waves-block waves-light">
                 <img class="activator responsive-img" src="images/sample-card-logo.jpg">
               </div>
               <div class="card-content">
                 <h3 class="card-title blue-text activator">Operating Deflection Shape (ODS) for Machinery Troubleshooting</h3>
               </div>
               <div class="card-reveal">
                  <span class="card-title"><i class="icon-close right"></i></span>
                  <p class="blue-text">
                    Vibration Condition Monitoring Surveillance at PT. Conocophillips Indonesia
                  </p>
                  <p>
                    Routine Condition Monitoring of Rotating and Electrical Equipment including Vibration, Partial Discharge and TAN Delta Analysis
                    <br />
                    <small class="text-muted">2009-2020</small>
                  </p>
                </div>
             </div>
           </div>
         </div>
         <div class="text-center">
           <a href="#" class="waves-effect waves-light btn btn-large btn-flat btn-outline-primary">Load More</a>
         </div>
         <br />
       </div>
     </section>
     <section>
         <div class="container">
           <h2 class="title"><span></span>See Our Client Testimonials</h2>
           <div class="slick-wrapper">
             <div class="ben-slick padd-slick">
               <div class="text-center">
                 <p>
                   Quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariaturLorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation
                 </p>
                 <i class="icon-quote amber-text"></i>
                 <p>
                   <strong>Pertamina</strong>
                 </p>
               </div>
               <div class="text-center">
                 <p>
                   Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariaturLorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation
                 </p>
                 <i class="icon-quote amber-text"></i>
                 <p>
                   <strong>Pertamina</strong>
                 </p>
               </div>
               <div class="text-center">
                 <p>
                   Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariaturLorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation
                 </p>
                 <i class="icon-quote amber-text"></i>
                 <p>
                   <strong>Pertamina</strong>
                 </p>
               </div>
               <div class="text-center">
                 <p>
                   Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariaturLorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation
                 </p>
                 <i class="icon-quote amber-text"></i>
                 <p>
                   <strong>Pertamina</strong>
                 </p>
               </div>
             </div>
           </div>
         </div>
     </section>
  </div>

  <div id="comp">
    <div class="hide-xs">
      <script src='https://api.mapbox.com/mapbox-gl-js/v0.38.0/mapbox-gl.js'></script>
        <link href='https://api.mapbox.com/mapbox-gl-js/v0.38.0/mapbox-gl.css' rel='stylesheet' />
        <div id='map' style='width: 100%; height: 500px;'></div>
       <script>
         mapboxgl.accessToken = 'pk.eyJ1IjoicmFtcG9rZ2VsbyIsImEiOiJjajVkbnlvcG0wbzFqMzNvNjgybWk2a3hwIn0.nBiU6cWxmYz3PjjIvrn75A';
         var map = new mapboxgl.Map({
         container: 'map',
         style: 'mapbox://styles/rampokgelo/cj5do371n0mgr2robfwt4x9xo'
         });
         map.scrollZoom.disable();
       </script>
    </div>
     <section class="section grey lighten-4">
       <div class="container">
         <div class="row">
           <div class="col s12 l4">
             <div class="card small">
               <div class="card-image card-image waves-effect waves-block waves-light">
                 <img class="activator responsive-img" src="images/sample-card-logo.jpg">
               </div>
               <div class="card-content">
                 <h3 class="card-title blue-text activator">Operating Deflection Shape (ODS) for Machinery Troubleshooting</h3>
               </div>
               <div class="card-reveal">
                  <span class="card-title"><i class="icon-close right"></i></span>
                  <p class="blue-text">
                    Vibration Condition Monitoring Surveillance at PT. Conocophillips Indonesia
                  </p>
                  <p>
                    Routine Condition Monitoring of Rotating and Electrical Equipment including Vibration, Partial Discharge and TAN Delta Analysis
                    <br />
                    <small class="text-muted">2009-2020</small>
                  </p>
                </div>
             </div>
           </div>
           <div class="col s12 l4">
             <div class="card small">
               <div class="card-image card-image waves-effect waves-block waves-light">
                 <img class="activator responsive-img" src="images/sample-card-logo.jpg">
               </div>
               <div class="card-content">
                 <h3 class="card-title blue-text activator">Vibration Condition Monitoring Surveillance at PT. Conocophillips Indonesia</h3>
               </div>
               <div class="card-reveal">
                  <span class="card-title"><i class="icon-close right"></i></span>
                  <p class="blue-text">
                    Vibration Condition Monitoring Surveillance at PT. Conocophillips Indonesia
                  </p>
                  <p>
                    Routine Condition Monitoring of Rotating and Electrical Equipment including Vibration, Partial Discharge and TAN Delta Analysis
                    <br />
                    <small class="text-muted">2009-2020</small>
                  </p>
                </div>
             </div>
           </div>
           <div class="col s12 l4">
             <div class="card small">
               <div class="card-image card-image waves-effect waves-block waves-light">
                 <img class="activator responsive-img" src="images/sample-card-logo.jpg">
               </div>
               <div class="card-content">
                 <h3 class="card-title blue-text activator">Operating Deflection Shape (ODS) for Machinery Troubleshooting</h3>
               </div>
               <div class="card-reveal">
                  <span class="card-title"><i class="icon-close right"></i></span>
                  <p class="blue-text">
                    Vibration Condition Monitoring Surveillance at PT. Conocophillips Indonesia
                  </p>
                  <p>
                    Routine Condition Monitoring of Rotating and Electrical Equipment including Vibration, Partial Discharge and TAN Delta Analysis
                    <br />
                    <small class="text-muted">2009-2020</small>
                  </p>
                </div>
             </div>
           </div>
           <div class="col s12 l4">
             <div class="card small">
               <div class="card-image card-image waves-effect waves-block waves-light">
                 <img class="activator responsive-img" src="images/sample-card-logo.jpg">
               </div>
               <div class="card-content">
                 <h3 class="card-title blue-text activator">Operating Deflection Shape (ODS) for Machinery Troubleshooting</h3>
               </div>
               <div class="card-reveal">
                  <span class="card-title"><i class="icon-close right"></i></span>
                  <p class="blue-text">
                    Vibration Condition Monitoring Surveillance at PT. Conocophillips Indonesia
                  </p>
                  <p>
                    Routine Condition Monitoring of Rotating and Electrical Equipment including Vibration, Partial Discharge and TAN Delta Analysis
                    <br />
                    <small class="text-muted">2009-2020</small>
                  </p>
                </div>
             </div>
           </div>
           <div class="col s12 l4">
             <div class="card small">
               <div class="card-image card-image waves-effect waves-block waves-light">
                 <img class="activator responsive-img" src="images/sample-card-logo.jpg">
               </div>
               <div class="card-content">
                 <h3 class="card-title blue-text activator">Operating Deflection Shape (ODS) for Machinery Troubleshooting</h3>
               </div>
               <div class="card-reveal">
                  <span class="card-title"><i class="icon-close right"></i></span>
                  <p class="blue-text">
                    Vibration Condition Monitoring Surveillance at PT. Conocophillips Indonesia
                  </p>
                  <p>
                    Routine Condition Monitoring of Rotating and Electrical Equipment including Vibration, Partial Discharge and TAN Delta Analysis
                    <br />
                    <small class="text-muted">2009-2020</small>
                  </p>
                </div>
             </div>
           </div>
           <div class="col s12 l4">
             <div class="card small">
               <div class="card-image card-image waves-effect waves-block waves-light">
                 <img class="activator responsive-img" src="images/sample-card-logo.jpg">
               </div>
               <div class="card-content">
                 <h3 class="card-title blue-text activator">Operating Deflection Shape (ODS) for Machinery Troubleshooting</h3>
               </div>
               <div class="card-reveal">
                  <span class="card-title"><i class="icon-close right"></i></span>
                  <p class="blue-text">
                    Vibration Condition Monitoring Surveillance at PT. Conocophillips Indonesia
                  </p>
                  <p>
                    Routine Condition Monitoring of Rotating and Electrical Equipment including Vibration, Partial Discharge and TAN Delta Analysis
                    <br />
                    <small class="text-muted">2009-2020</small>
                  </p>
                </div>
             </div>
           </div>
           <div class="col s12 l4">
             <div class="card small">
               <div class="card-image card-image waves-effect waves-block waves-light">
                 <img class="activator responsive-img" src="images/sample-card-logo.jpg">
               </div>
               <div class="card-content">
                 <h3 class="card-title blue-text activator">Operating Deflection Shape (ODS) for Machinery Troubleshooting</h3>
               </div>
               <div class="card-reveal">
                  <span class="card-title"><i class="icon-close right"></i></span>
                  <p class="blue-text">
                    Vibration Condition Monitoring Surveillance at PT. Conocophillips Indonesia
                  </p>
                  <p>
                    Routine Condition Monitoring of Rotating and Electrical Equipment including Vibration, Partial Discharge and TAN Delta Analysis
                    <br />
                    <small class="text-muted">2009-2020</small>
                  </p>
                </div>
             </div>
           </div>
         </div>
         <div class="text-center">
           <a href="#" class="waves-effect waves-light btn btn-large btn-flat btn-outline-primary">Load More</a>
         </div>
         <br />
       </div>
     </section>
     <section>
         <div class="container">
           <h2 class="title"><span></span>See Our Client Testimonials</h2>
           <div class="slick-wrapper">
             <div class="ben-slick padd-slick">
               <div class="text-center">
                 <p>
                   Quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariaturLorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation
                 </p>
                 <i class="icon-quote amber-text"></i>
                 <p>
                   <strong>Pertamina</strong>
                 </p>
               </div>
               <div class="text-center">
                 <p>
                   Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariaturLorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation
                 </p>
                 <i class="icon-quote amber-text"></i>
                 <p>
                   <strong>Pertamina</strong>
                 </p>
               </div>
               <div class="text-center">
                 <p>
                   Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariaturLorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation
                 </p>
                 <i class="icon-quote amber-text"></i>
                 <p>
                   <strong>Pertamina</strong>
                 </p>
               </div>
               <div class="text-center">
                 <p>
                   Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariaturLorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation
                 </p>
                 <i class="icon-quote amber-text"></i>
                 <p>
                   <strong>Pertamina</strong>
                 </p>
               </div>
             </div>
           </div>
         </div>
     </section>
  </div>


 </div>




 <?php
 include 'templates/modals.php';
  ?>
 <?php
include 'templates/footer.php';
  ?>
